...
export class SignalsPlugin {
  ...
  private attachListeners(): void {
    ...
    const customInput = document.getElementById('custom-signal-input') as HTMLInputElement;
    ...
    runtimeClient.on('pipeline.event', (event) => {
      if (event.type === 'signal.received') {
        this.showStatus('Signal injected: ' + (event as any).raw_content, 'success');
      }
    });

    runtimeClient.on('runtime.error', (error) => {
      this.showStatus((error as any).error, 'error');
    });
  }
...
export const signals = new SignalsPlugin();
