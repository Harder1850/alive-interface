...
exports.launcher = exports.LauncherPlugin = void 0;
...
class LauncherPlugin {
    ...
    attachListeners() {
        ...
        runtime_client_1.runtimeClient.on('runtime.started', () => this.updateStatus());
        runtime_client_1.runtimeClient.on('runtime.stopped', () => this.updateStatus());
        runtime_client_1.runtimeClient.on('status.updated', (status) => this.displayStatus(status));
        runtime_client_1.runtimeClient.on('pipeline.event', () => this.updateStatus());
    }
    async start(profile) {
        try {
            console.log(`[LauncherPlugin] Starting with profile: ${profile}`);
            await runtime_client_1.runtimeClient.executeCommand({ type: 'start', profile });
            this.isRunning = true;
    ...
    async stop() {
        try {
            ...
            await runtime_client_1.runtimeClient.executeCommand({ type: 'stop' });
            this.isRunning = false;
            this.updateButtonStates();
        }
        catch (error) {
...
exports.LauncherPlugin = LauncherPlugin;
exports.launcher = new LauncherPlugin();
